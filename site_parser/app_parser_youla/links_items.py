        ...
        async with session.post(url=url_api, headers=headers, json=json_data) as r:
            ...
            for url in item_urls:
                ...
                #     pass
                    ...
                    if url_item not in variables.links_item:  # борьба с дублями
                        if url_item != None:
                            # print(f"{id_category} {url_item}")
                            variables.links_item.append(url_item)
                            variables.find_links = variables.find_links + 1  # Найдено ссылок
                            # print(f"Найдено ссылок {variables.find_links} {url_item}")
                        else: pass
                    #
...
def main():
    ...
    asyncio.new_event_loop().run_until_complete(gahter_links_items())


    # loop = asyncio.new_event_loop()
    # loop.run_until_complete(gahter_links_items())
...
if __name__ == '__main__':
    ...
    with open('links_item.txt', 'w') as f:
        f.write(str(variables.links_item))
